...
public class Element3D8 : AbstraktLinear3D8
{
    ...
    private readonly double[] _elementDeformations = new double[24];// an Elementknoten
    private readonly double[,] _b = new double[6, 24];               // Spannungs-Dehnungs-Transformation
    private readonly double[,] _e = new double[6, 6];                // Materialmatrix
    private double _z0, _z1, _z2, _g0, _g1, _g2;
    // Gausskoordinaten und -gewichte
    private static readonly double[] GCoord = [-1.0 / Math.Sqrt(5.0 / 3.0), 0.0, 1.0 / Math.Sqrt(5.0 / 3.0)];
    private static readonly double[] GWeight = [5.0 / 9.0, 8.0 / 9.0, 5.0 / 9.0];
    ...
    public override double[,] BerechneElementMatrix()
    {
        ...
        {
            _z0 = GCoord[i]; _g0 = GWeight[i];
            for (var j = 0; j < GCoord.Length; j++)
            {
                _z1 = GCoord[j]; _g1 = GWeight[j];
                for (var k = 0; k < GCoord.Length; k++)
                {
                    _z2 = GCoord[k]; _g2 = GWeight[k];
    ...
    private void BerechneSpannungsDehnungsTransformation()
    {
        int i, j, k;
        for (i = 0; i < 3; i++)
            for (j = i, k = 0; k < 8; j += 3, k++) _b[i, j] = Sx[k, i];

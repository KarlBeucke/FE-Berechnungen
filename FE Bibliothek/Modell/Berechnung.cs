...
namespace FEBibliothek.Modell
{
    public class Berechnung
    {
        ...
        private void SetzAnfangsbedingungenErsterOrdnung(IList<double[]> temperatur)
        {
            ...
            for (var i1 = 0; i1 < _modell.Zeitintegration.Anfangsbedingungen.Count; i1++)
            {
                var anf = (Knotenwerte)_modell.Zeitintegration.Anfangsbedingungen[i1];
        ...
        private double[] BerechneDämpfungsMatrix()
        {
            ...
            for (var n = 0; n < _modell.Eigenzustand.AnzahlZustände; n++)
            {
                ...
                double mn = _systemGleichungen.DiagonalMatrix.Select((t, i) 
                    => _modell.Eigenzustand.Eigenvektoren[n][i] * t * _modell.Eigenzustand.Eigenvektoren[n][i]).Sum();
        ...
        private void SetzRandbedingungenZweiterOrdnung(IReadOnlyList<double[]> displ, IReadOnlyList<double[]> veloc)
        {
            ...
            foreach (var anf in _modell.Zeitintegration.Anfangsbedingungen.Cast<Knotenwerte>())
            {
